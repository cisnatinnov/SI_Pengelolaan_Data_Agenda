...
const realisasiLabels = {
    terlaksana: { text: 'Terlaksana', class: 'bg-emerald-500/15 text-emerald-700 ' },
    tidak: { text: 'Tidak Terlaksana', class: 'bg-red-500/15 text-red-700 ' },
};

const statusLabels = {
    pelaksanaan: { text: 'Pelaksanaan', class: 'bg-cyan-500/15 text-cyan-700 ' },
    laporan: { text: 'Laporan', class: 'bg-indigo-500/15 text-indigo-700 ' },
};
                ...
                <table class="min-w-full divide-y divide-slate-200 ">
                    <thead class="bg-slate-50/70 ">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500  uppercase tracking-wider">Nama Kegiatan</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500  uppercase tracking-wider">Tempat</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500  uppercase tracking-wider">Tanggal</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500  uppercase tracking-wider">Uraian</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500  uppercase tracking-wider">Realisasi</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500  uppercase tracking-wider">Status</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500  uppercase tracking-wider">Kehadiran</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500  uppercase tracking-wider">Penyusun</th>
                            <th v-if="canManage" class="px-6 py-3 text-right text-xs font-medium text-slate-500  uppercase tracking-wider">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200 ">
                        ...
                        >
                            ...
                            <td class="px-6 py-4 text-sm max-w-xs">
                                ...
                                <p v-if="item.keterangan" class="mt-0.5 text-xs text-slate-400  truncate" :title="item.keterangan">
                                    {{ item.keterangan }}
                                </p>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <span
                                    ...
                                    :class="realisasiLabels[item.realisasi_pelaksanaan]?.class ?? 'bg-slate-500/10 text-slate-600 '"
                                >
                            ...
                            <td class="px-6 py-4 text-sm">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <button
                                            ...
                                            :class="ownKehadiran(item) === 'hadir'
                                                ...
                                                : 'bg-slate-100  text-slate-600  border-transparent hover:bg-emerald-500/10 hover:text-emerald-600 '"
                                        >
                                            Hadir
                                        </button>
                                        <button
                                            ...
                                            :class="ownKehadiran(item) === 'tidak'
                                                ...
                                                : 'bg-slate-100  text-slate-600  border-transparent hover:bg-red-500/10 hover:text-red-600 '"
                                        >
                                            Tidak Hadir
                                        </button>
                                    </div>
                                ...
                                <span v-else class="text-sm text-slate-500 ">
                                    ...
                                    <ul v-if="kehadiranListId === item.id" class="mt-2 space-y-1">
                                        ...
                                        >
                                            <span class="text-slate-600 ">{{ k.user?.name ?? 'OPD' }}</span>
                                            <span
